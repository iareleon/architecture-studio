#!/usr/bin/env python3
"""install.py — idempotent installer for SkillsLoom

Safe to run multiple times. Never overwrites existing skill data.
Reads config from ~/.skillsloom/config.yaml if it already exists (or legacy ~/.skillmanager/config.yaml).

Usage: python3 scripts/install.py
Override defaults: SKILLSLOOM_DIR=/custom/path python3 scripts/install.py
(Legacy alias: SKILLMANAGER_DIR)
"""
...
def main() -> None:
    ...
    header("SkillsLoom Installer")
    print("\nThis installer will set up SkillsLoom on your system.")
    ...
    # Install directory
    env_dir = (os.environ.get("SKILLSLOOM_DIR", "") or os.environ.get("SKILLMANAGER_DIR", "")).strip()
    if env_dir:
        info(f"Using install directory override: {env_dir}")
        skillmanager_dir = Path(env_dir.replace("~", str(_HOME)))
    else:
        default_dir = _get_default_install_dir()
        user_input = input(f"Where should SkillsLoom assets live? [{default_dir}]: ").strip()
    ...
    while (skillmanager_dir / ".git").is_dir():
        warn(f"'{skillmanager_dir}' is a git repository. SkillsLoom must be installed outside any cloned repo.")
    ...
    config_file = skillmanager_dir / "config.yaml"
    ...
    print("  [M] Manual     — invoke /skill-manager or /brain-manager when you need them\n")
    ...
    config_content = (
        "# SkillsLoom configuration — generated by install.py\n"
    ...
    if model_file.exists():
        ...
        print(f"\nSkillsLoom uses a personal persona file to set your AI assistant identity.")
    ...
    # -----------------------------------------------------------------------
    header("Step 8: Installing SkillsLoom CLI")
    ...
    print(f"  SKILLSLOOM_DIR=/tmp/sf-test python3 {script_dir}/install.py")
    print()
